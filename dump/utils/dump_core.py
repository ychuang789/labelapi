...
from settings import CONFLICT_GROUPS, DatabaseConfig, TABLE_PREFIX, TABLE_GROUPS_FOR_INDEX
...
def run(generate_dict: Dict[str,List[str]], result_table_dict: Dict[str,List[str]],
        input_database: str, output_database: str, prefix: str,
    ...
    for table_name, task_ids in tqdm(generate_dict.items()):
        ...
        logger.info(f'start scraping old data...')
        previous_table_list = get_old_table_list(input_database, get_table_suffix_condition())
        temp_table_name =  prefix + table_name
