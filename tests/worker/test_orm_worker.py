...
class TestModelORMWorker(TestCase):
    ...
    def test_status_changer(self):
        er = self.orm_cls.model_status_changer(task_id=self.model_job_id)
        self.assertIsInstance(er, str)

    def test_get_status(self):
        record = self.orm_cls.model_get_status(task_id=self.model_job_id)
        self.assertIsNotNone(record)

    def test_get_report(self):
        report = self.orm_cls.model_get_report(task_id=self.model_job_id)
